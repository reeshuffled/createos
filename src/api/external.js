...
class WeatherSignal {
  ...
  get temperature()   { return this._data.temperature_2m   ?? null; }
  get windSpeed()     { return this._data.wind_speed_10m   ?? null; }
  get precipitation() { return this._data.precipitation    ?? null; }
  get humidity()      { return this._data.relative_humidity_2m ?? null; }
  get raw()           { return this._data; }

  async _fetch() {
    const url = `${OPEN_METEO}?latitude=${this._lat}&longitude=${this._lon}` +
  ...
  stream(fn, intervalMs = 60_000) {
    ...
    const id = setInterval(async () => {
      try { await this._fetch(); fn(this); } catch (_) {}
    }, intervalMs);
