...
let _masterIn = null, _masterVol = null, _masterEQ = null, _masterMeter = null;
...
function _ensureSettingsLoaded() { if (!_settingsLoaded) { _settingsLoaded = true; _loadSettings(); } }
...
function _defaults() { return { volume: 0, pan: 0, mute: false, solo: false, eq: null }; }
...
class EQChain {
  constructor() {
    ...
    this.nodes = this.bands.map(b => new Tone.Filter({ type: b.type, frequency: b.freq, Q: b.q, gain: b.gain }));
  }
  _apply(bands) {
    if (!bands) return;
    bands.forEach((b, i) => {
  ...
  serialize() { return this.bands.map(b => ({ ...b })); }
  dispose() { this.nodes.forEach(n => { try { n.dispose(); } catch (_) {} }); }
}
...
class Strip {
  ...
  get input() { return this._in; }          // sources connect here (Tone node)
  // Connect a raw/native AudioNode into this strip.
  connectFrom(node) { try { Tone.connect(node, this._in); } catch (_) {} return this; }

  _settings() { return _settingsFor(this.name); }

  _applySettings() {
    const s = this._settings();
    try { this._channel.volume.value = s.volume; } catch (_) {}
    try { this._channel.pan.value = s.pan; } catch (_) {}
  ...
  _ensureEQ() {
    ...
    try { this._in.disconnect(this._channel); } catch (_) {}
    this._in.chain(...this._eq.nodes, this._channel);
    return this._eq;
  }

  // ── setters (return this; blocks-friendly) ──────────────────────────────────
  setVolume(db)  { this._settings().volume = db;  try { this._channel.volume.value = db; } catch (_) {} _persistAndNotify(); return this; }
  setPan(v)      { this._settings().pan = v;       try { this._channel.pan.value = v; } catch (_) {} _persistAndNotify(); return this; }
  setMute(b)     { this._settings().mute = !!b;    _recomputeDucking(); _persistAndNotify(); return this; }
  setSolo(b)     { this._settings().solo = !!b;    _recomputeDucking(); _persistAndNotify(); return this; }
  setEQ(bands)   { this._ensureEQ()._apply(bands); this._settings().eq = this._eq.serialize(); _persistAndNotify(); return this; }

  _level() { try { const v = this._meter.getValue(); return typeof v === 'number' ? v : (v?.[0] ?? -Infinity); } catch (_) { return -Infinity; } }

  dispose() {
    try { this._in.dispose(); } catch (_) {}
    try { this._channel.dispose(); } catch (_) {}
...
function _recomputeDucking() {
  ...
  for (const [name, strip] of _strips) {
    ...
    try { strip._channel.mute = muted; } catch (_) {}
  }
  // master never ducks; it only obeys its own mute
  if (_masterVol) {
    const ms = _settingsFor('master');
    try { _masterVol.volume.value = ms.mute ? -Infinity : ms.volume; } catch (_) {}
  }
}

// ── Registry / discovery ───────────────────────────────────────────────────────

function _emitChange() { for (const fn of _changeListeners) { try { fn(); } catch (_) {} } }
function _persistAndNotify() { _saveSettings(); _emitChange(); }
...
export function getStrip(name) { return _strips.get(name) || null; }
export function liveStripNames() { return [..._strips.keys()]; }
...
subscribe('pattern:started', ({ id, strip: stripName }) => {
  if (!stripName) return;
  _strips.get(stripName)?.patterns.add(id);
  _emitChange();
}, { persistent: true });
subscribe('pattern:stopped', ({ id }) => {
...
class StripHandle {
  ...
  _live() { return _strips.get(this._name) || null; }
  _set() { return _settingsFor(this._name); }
  volume(db) { const l = this._live(); if (l) l.setVolume(db); else { this._set().volume = db; _persistAndNotify(); } return this; }
  pan(v)     { const l = this._live(); if (l) l.setPan(v);   else { this._set().pan = v; _persistAndNotify(); } return this; }
  mute(b = true)  { const l = this._live(); if (l) l.setMute(b); else { this._set().mute = !!b; _recomputeDucking(); _persistAndNotify(); } return this; }
  solo(b = true)  { const l = this._live(); if (l) l.setSolo(b); else { this._set().solo = !!b; _recomputeDucking(); _persistAndNotify(); } return this; }
  eq(bands)  { const l = this._live(); if (l) l.setEQ(bands); else { this._set().eq = bands; _persistAndNotify(); } return this; }
  get name() { return this._name; }
}

class MasterHandle extends StripHandle {
  constructor() { super('master'); }
  volume(db) { _ensureMaster(); this._set().volume = db; try { _masterVol.volume.value = this._set().mute ? -Infinity : db; } catch (_) {} _persistAndNotify(); return this; }
  mute(b = true) { _ensureMaster(); this._set().mute = !!b; _recomputeDucking(); _persistAndNotify(); return this; }
  pan() { return this; }   // master is post-sum; pan is a no-op
  solo() { return this; }  // master never solos
  eq(bands) { _ensureMaster(); _ensureMasterEQ()._apply(bands); this._set().eq = _masterEQ.serialize(); _persistAndNotify(); return this; }
}

class Mixer {
  show()   { _ensurePanel(); _panelSet(true);  return this; }
  hide()   { _panelSet(false); return this; }
  toggle() { _ensurePanel(); _panelSet(!_panelVisible()); return this; }
  strip(name) { return new StripHandle(name); }
  get master() { return new MasterHandle(); }
  ...
  add(node, { name, persist } = {}) {
    const strip = acquireStrip(name, { type: 'node', owner: window.__ar_active_editor_id ?? null, lifecycle: persist ? 'persistent' : 'run' });
    try {
      if (node?.connect && node?.toDestination) node.connect(strip.input);   // Tone node
      else if (node) strip.connectFrom(node);                                // raw AudioNode
    } catch (_) {}
    return new StripHandle(strip.name);
  }
  names() { return liveStripNames(); }
}
...
function _panelVisible() { const w = _panel && document.getElementById(_panel.winId); return !!w && w.style.display !== 'none'; }
function _panelSet(v) {
  if (!_panel) return;
  const w = document.getElementById(_panel.winId);
  if (w) w.style.display = v ? 'flex' : 'none';
  if (v) _startPanelLoop(); else _stopPanelLoop();
}
...
function _ensurePanel() {
  ...
  if (!body) return;
  body.style.cssText += 'background:#0d0d14;overflow-x:auto;overflow-y:hidden;padding:6px;display:flex;gap:6px;align-items:stretch;';
  ...
  if (win) win._wmCleanup = () => { _stopPanelLoop(); _changeListeners.delete(_renderPanel); _panel = null; };
...
function _renderStrip(name, isMaster, orphan = false) {
  ...
  title.title = name;
  title.style.cssText = 'font:600 11px monospace;color:#cdd6f4;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;';
  if (!isMaster && !orphan) {
  ...
  if (strip && strip.patterns.size) {
    ...
    sub.style.cssText = 'font:9px monospace;color:#7f849c;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;';
  ...
  meterFill.style.cssText = 'height:100%;width:0%;background:linear-gradient(90deg,#a6e3a1,#f9e2af,#f38ba8);';
  meterWrap.appendChild(meterFill);
  col.appendChild(meterWrap);
  _meterEls.set(name, meterFill);

  // volume
  col.appendChild(_slider('vol', -48, 6, 0.5, s.volume, v => _handleFor(name, isMaster).volume(v), `${s.volume>0?'+':''}${s.volume.toFixed(1)}dB`));
  // pan (not master)
  if (!isMaster) col.appendChild(_slider('pan', -1, 1, 0.05, s.pan, v => _handleFor(name).pan(v), `pan ${s.pan.toFixed(2)}`));
  ...
  // EQ toggle
  const eqBtn = _toggleBtn('EQ', !!s.eq, '#89b4fa', () => { _handleFor(name, isMaster).eq(s.eq || EQ_DEFAULT.map(b => ({ ...b }))); _renderPanel(); });
  ...
  if (orphan) {
    ...
    x.textContent = '✕ clear';
    x.style.cssText = 'font:9px monospace;background:#23233a;color:#bac2de;border:none;border-radius:3px;cursor:pointer;padding:2px;';
    x.onclick = () => { _settings.delete(name); _saveSettings(); _renderPanel(); };
    col.appendChild(x);
  }
...
function _handleFor(name, isMaster) { return isMaster ? mixer.master : mixer.strip(name); }
...
function _slider(kind, min, max, step, value, onInput, label) {
  ...
  r.addEventListener('input', e => { e.stopPropagation(); onInput(parseFloat(r.value)); lab.textContent = _relabel(kind, parseFloat(r.value)); });
...
function _eqCanvas(name, isMaster, bands) {
  ...
  c.style.cssText = 'width:100%;height:60px;display:block;background:#080810;border-radius:4px;cursor:ns-resize;margin-top:2px;';
  const W = () => c.width, H = () => c.height;
  const ro = new ResizeObserver(() => { c.width = c.offsetWidth * devicePixelRatio; c.height = c.offsetHeight * devicePixelRatio; _drawEQ(); });
  ro.observe(c);
  const f2x = f => Math.log10(f / 20) / Math.log10(1000) * W();
  const db2y = db => H() / 2 - (db / 12) * (H() / 2);
  function _drawEQ() {
    const ctx = c.getContext('2d'); if (!ctx || !W()) return;
    ctx.clearRect(0, 0, W(), H());
    ctx.strokeStyle = '#1e1e2e'; const y0 = db2y(0);
    ctx.beginPath(); ctx.moveTo(0, y0); ctx.lineTo(W(), y0); ctx.stroke();
    ctx.strokeStyle = '#89b4fa'; ctx.lineWidth = 1.5 * devicePixelRatio;
    ctx.beginPath();
    ...
    ctx.stroke();
    for (const b of bands) {
      ctx.beginPath(); ctx.arc(f2x(b.freq), db2y(b.gain), 4 * devicePixelRatio, 0, 7); ctx.fillStyle = '#f9e2af'; ctx.fill();
    }
  }
  let drag = null;
  c.addEventListener('mousedown', e => {
    ...
    bands.forEach((b, i) => { if (Math.abs(f2x(b.freq) - mx) < 16 * devicePixelRatio) drag = { i, startY: e.clientY, startG: b.gain }; });
  ...
  c._eqCleanup = () => { document.removeEventListener('mousemove', mv); document.removeEventListener('mouseup', up); };
  _drawEQ();
  return c;
}
...
function _startPanelLoop() {
  ...
  const tick = () => {
    ...
    for (const [name, el] of _meterEls) {
      let db = -Infinity;
      if (name === 'master') { try { db = _masterMeter?.getValue() ?? -Infinity; if (typeof db !== 'number') db = db?.[0] ?? -Infinity; } catch (_) {} }
...
function _stopPanelLoop() {
  ...
  _panel.rafId = null;
  _panel.live?.release(); _panel.live = null;
}
...
function _renameStrip(oldName, newName) {
  if (_settings.has(oldName)) { _settings.set(newName, _settings.get(oldName)); _settings.delete(oldName); }
  const strip = _strips.get(oldName);
  if (strip) { strip.name = newName; _strips.delete(oldName); _strips.set(newName, strip); }
  _saveSettings();
  _renderPanel();
}

// Panel open helper for the toolbar button / restorer.
export function openMixerPanel() { mixer.show(); }
