...
describe('pat_create', () => {
  test('generates pattern() call', () => {
    const b = makeBlock({ NOTES: 'C4 E4 G4' });
    const g = makeGen({ SYNTH: 'synth' });
    expect(gen('pat_create', b, g)).toEqual([`pattern("C4 E4 G4", synth)`, 2]);
  });
  test('defaults synth to null', () => {
    const b = makeBlock({ NOTES: 'C4' });
    expect(gen('pat_create', b)[ 0]).toBe(`pattern("C4", null)`);
  });
});

describe('pat_chord', () => {
  test('joins notes as comma-separated string', () => {
    const b = makeBlock({ NOTES: 'C4 E4 G4' });
    const g = makeGen({ SYNTH: 's' });
    expect(gen('pat_chord', b, g)[0]).toBe(`audio.chord(["C4","E4","G4"], s)`);
  });
});

describe('pat_start', () => {
  test('generates bpm+start chain', () => {
    const b = makeBlock({ BPM: '130' });
    const g = makeGen({ PAT: 'myPat' });
    expect(gen('pat_start', b, g)).toBe('(myPat).bpm(130).start();\naudio.start();\n');
  });
});

describe('pat_speed', () => {
  test('generates .speed() chain', () => {
    const b = makeBlock({ N: '2' });
    const g = makeGen({ PAT: 'p' });
    expect(gen('pat_speed', b, g)[0]).toBe('(p).speed(2)');
  });
});

describe('pat_reverse', () => {
  test('generates .reverse() chain', () => {
    const g = makeGen({ PAT: 'p' });
    expect(gen('pat_reverse', makeBlock(), g)[0]).toBe('(p).reverse()');
  });
});

describe('pat_transpose', () => {
  test('generates .transpose() chain', () => {
    const b = makeBlock({ N: '7' });
    const g = makeGen({ PAT: 'p' });
    expect(gen('pat_transpose', b, g)[0]).toBe('(p).transpose(7)');
  });
});

describe('pat_rhythm', () => {
  test('generates .rhythm() chain', () => {
    const b = makeBlock({ K: '3', N: '8' });
    const g = makeGen({ PAT: 'p' });
    expect(gen('pat_rhythm', b, g)[0]).toBe('(p).rhythm(3, 8)');
  });
});
